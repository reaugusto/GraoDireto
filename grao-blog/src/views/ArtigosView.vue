<template>
    <div id="container">
        <div class="topo">
            <h2 @click="ListarArtigos()">Todos os artigos</h2>
            <button class="criar" @click="$router.push({ name: 'postar', params: { id: 0 } })">Criar Artigo</button>
        </div>
        <div class="listTag">
...
import { onMounted, ref, computed } from 'vue'; // Adicionado computed
...
// Lógica de Fatiamento (Pagination)
const artigosPaginados = computed(() => {
    const inicio = (paginaAtual.value - 1) * itensPorPagina.value;
    const fim = inicio + itensPorPagina.value;
    return artigos.value.slice(inicio, fim);
});

const totalPaginas = computed(() => {
    return Math.ceil(artigos.value.length / itensPorPagina.value);
});
...
async function ListarArtigos() {
    try {
        ...
        artigos.value = data;
        paginaAtual.value = 1; // Reseta para a primeira página ao carregar novos dados
    } catch (error) {
        console.error(error);
    }
};

async function ListarArtigosTag(tagId) {
    try {
        const { data } = await http.get(`/artigos/tag/${tagId}`);
        artigos.value = data;
        paginaAtual.value = 1; // Reseta ao filtrar
    } catch (error) {
        console.error(error);
    }
};
...
async function Buscar() {
    if (termo.value && termo.value.length > 3) {
        try {
...
function formatarTags(tagsString) {
    ...
    if (typeof (tagsString) != 'string') return tagsString;
